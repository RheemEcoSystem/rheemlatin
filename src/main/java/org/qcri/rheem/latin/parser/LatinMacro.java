...
import org.qcri.rheem.latin.exception.ParserException;
import org.qcri.rheem.latin.util.LatinContext;

import java.io.*;
import java.util.*;

public class LatinMacro {

    private static final Log LOG = LogFactory.getLog(LatinMacro.class);
    ...
    private Map<String, LatinMacro> seen;
    ...
    public LatinMacro(String name, String file, List<String> params,
               List<String> returns, String body, Map<String, LatinMacro> seen) {
        this.name = name;
    ...
    void setLatinContext(LatinContext latinContext) {
   ...
   /**         PreprocessorContext pc = new PreprocessorContext(50);
            ...
            for (Map.Entry<String, String> e : latinContext.getParamVal().entrySet()) {
          ...
          //  throw new Exception("Problema aqui 1");
        } catch (Exception e) {
    ...
    private CommonTree maskAlias(String in, Set<String> masks, CommonTree tree,
        ...
        if (!macroDefNodes.isEmpty()) {
            String fname = ((LatinParserNode)ast).getFileName();
        ...
        LatinParserNode pnode = (LatinParserNode)tree;
        List<LatinParserNode.InvocationPoint> invStack = pnode.getInvocationStack();
        List<LatinParserNode.InvocationPoint> newInvStack = (invStack == null) ? new ArrayList<LatinParserNode.InvocationPoint>()
                : new ArrayList<LatinParserNode.InvocationPoint>(invStack);

        LatinParserNode.InvocationPoint pt = new LatinParserNode.InvocationPoint(line, file, name);
        newInvStack.add(pt);
        ...
        for (CommonTree t : inlineNodes) {
            CommonTree newTree = macroInline(t,
                    new ArrayList<LatinMacro>(seen.values()), macroStack, latinContext);
    ...
    private static void setInvocationStack(Tree ast, List<LatinParserNode.InvocationPoint> stack) {
        LatinParserNode node = (LatinParserNode)ast;
     ...
     */
    public static CommonTree macroInline(CommonTree t,
                                  List<LatinMacro> macroDefs, Set<String> macroStack,
                                  LatinContext latinContext)
                                  throws ParserException {
        // get name
        String mn = t.getChild(0).getText();

        // get macroDef
        LatinMacro macro = null;
        for (LatinMacro pm : macroDefs) {
        ...
        String file = ((LatinParserNode)t).getFileName();
        ...
        // inform the macro of the LatinContext
        // so it can substitute parameters from the main pigscript
        macro.setLatinContext(latinContext);
