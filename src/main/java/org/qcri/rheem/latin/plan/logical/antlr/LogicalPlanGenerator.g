...
/**
 * Grammar file for Latin tree parser (for schema alias validation).
 *
...
@header {
package org.apache.latin.plan.logical;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.org.qcri.rheem.latin.impl.LatinContext;
import org.org.qcri.rheem.latin.impl.builtin.GFAny;
import org.org.qcri.rheem.latin.impl.logicalLayer.FrontendException;
import org.org.qcri.rheem.latin.impl.streaming.StreamingCommand;
import org.org.qcri.rheem.latin.impl.streaming.StreamingCommand.HandleSpec;
import org.org.qcri.rheem.latin.impl.util.MultiMap;
import org.org.qcri.rheem.latin.impl.util.NumValCarrier;
import org.org.qcri.rheem.latin.impl.plan.PlanValidationException;
import org.org.qcri.rheem.latin.newplan.Operator;
import org.qcri.rheem.latin.plan.operator.expression.le.AddExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.AndExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.BinCondExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.CastExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.ConstantExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.DereferenceExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.DivideExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.EqualExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.GreaterThanEqualExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.GreaterThanExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.IsNullExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.LessThanEqualExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.LessThanExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.MapLookupExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.ModExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.MultiplyExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.NegativeExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.NotEqualExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.NotExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.OrExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.RegexExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.ScalarExpression;
import org.qcri.rheem.latin.plan.operator.expression.le.SubtractExpression;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOCogroup;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOCube;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOFilter;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOForEach;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOGenerate;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOLimit;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOJoin;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOSort;
import org.qcri.rheem.latin.plan.operator.relational.lo.LORank;
import org.qcri.rheem.latin.plan.operator.relational.lo.LOSplitOutput;
...
import org.org.qcri.rheem.latin.data.DataBag;
import org.org.qcri.rheem.latin.DataType;
import org.org.qcri.rheem.latin.data.Tuple;
...
: ^(REALIAS alias IDENTIFIER)
    {
	    ...
	    if (op==null) {
	        throw new UndefinedAliasException(input,
	            new SourceLocation( (LAtinParserNode)$IDENTIFIER ), $IDENTIFIER.text);
	    }
...
@init {
    ...
    SourceLocation loc = new SourceLocation( (LAtinParserNode)$cmd.start );
}
...
load_clause returns[String alias]
 : ^( LOAD filename func_clause[FunctionType.LOADFUNC]? as_clause? )
  {
      SourceLocation loc = new SourceLocation( (LatinParserNode)$load_clause.start );
...
@init {
    ...
    SourceLocation loc = new SourceLocation( (LatinParserNode)$group_clause.start );
...
group_type returns[GROUPTYPE type]
 : QUOTEDSTRING
   {
       $type =builder.parseGroupType( $QUOTEDSTRING.text, new SourceLocation( (LatinParserNode)$QUOTEDSTRING ) );
   }
;
...
@init { boolean inner = false; }
 : rel ( join_group_by_clause
         ...
         | ALL
         {
             ...
             ce.setLocation( new SourceLocation( (LatinParserNode)$ALL ) );
...
previous_rel returns[String name] : ARROBA { $name = builder.getLastRel(new SourceLocation((LatinParserNode)$ARROBA)); }
;
...
@init {
 ...
 : ( flatten_clause[$plan] { $flattenFlag = true; }
   ...
   | STAR
     {
         builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$STAR ), $plan, $GScope::currentOp,
...
store_clause returns[String alias]
 : ^( STORE rel filename func_clause[FunctionType.STOREFUNC]? )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$STORE );
...
@init {
 ...
 : ^( ASSERT rel cond[exprPlan] comment? )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$ASSERT );
...
@init {
 ...
 : ^( FILTER rel cond[exprPlan] )
   {
       $alias = builder.buildFilterOp( new SourceLocation( (LatinParserNode)$FILTER ),
...
cond[LogicalExpressionPlan exprPlan] returns[LogicalExpression expr]
 : ^( OR left = cond[exprPlan] right = cond[exprPlan] )
   {
       ...
       $expr.setLocation( new SourceLocation( (LatinParserNode)$OR ) );
   }
 | ^( AND left = cond[exprPlan] right = cond[exprPlan] )
   {
       $expr = new AndExpression( $exprPlan, $left.expr, $right.expr );
       $expr.setLocation( new SourceLocation( (LatinParserNode)$AND ) );
   }
 | ^( NOT c = cond[exprPlan] )
   {
       $expr = new NotExpression( $exprPlan, $c.expr );
       $expr.setLocation( new SourceLocation( (LatinParserNode)$NOT ) );
   }
 | ^( NULL expr[$exprPlan] NOT? )
   {
       $expr = new IsNullExpression( $exprPlan, $expr.expr );
       $expr.setLocation( new SourceLocation( (LatinParserNode)$NULL ) );
       if( $NOT != null ) {
 ...
 | ^( BOOL_COND e1 = expr[$exprPlan] )
   {
       $expr = $e1.expr;
       $expr.setLocation( new SourceLocation( (LatinParserNode)$BOOL_COND ) );
   }
;
...
@init {
 ...
 : ^( IN ( ^( IN_LHS lhs = expr[$plan] ) { lhsExprs.add($lhs.expr); }
           ^( IN_RHS rhs = expr[$plan] ) { rhsExprs.add($rhs.expr); } )+ )
    {
        ...
        $expr.setLocation( new SourceLocation( (LatinParserNode)$in_eval.start ) );
    }
;
...
@init {
 ...
 : ^( FUNC_EVAL func_name ( real_arg[$plan] { args.add( $real_arg.expr ); } )* )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$func_name.start );
...
real_arg [LogicalExpressionPlan plan] returns[LogicalExpression expr]
 ...
 | STAR
   {
       $expr = builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$STAR ), $plan, $GScope::currentOp,
           $statement::inputIndex, null, -1 );
   }
 ...
 | ^( NEG e = expr[$plan] )
   {
       ...
       $expr.setLocation( new SourceLocation( (LatinParserNode)$e.start ) );
   }
 | ^( CAST_EXPR type_cast e = expr[$plan] ) // cast expr
   {
       $expr = new CastExpression( $plan, $e.expr, $type_cast.fieldSchema );
       $expr.setLocation( new SourceLocation( (LatinParserNode)$type_cast.start ) );
   }
...
@init {
    ...
    SourceLocation loc = new SourceLocation( (LatinParserNode)$var_expr.start );
}
   ...
   ( dot_proj
     {
         ...
         if( processScalar ) {
             ...
             e.setLocation( new SourceLocation( (LatinParserNode)$dot_proj.start ) );
             $plan.connect( e, $expr );
             $expr = e;
         }
     }
   | pound_proj
     {
         ...
         e.setLocation( new SourceLocation( (LatinParserNode)$pound_proj.start ) );
         $plan.connect( e, $expr );
         $expr = e;
     }
  )*
...
col_range[LogicalExpressionPlan plan] returns[LogicalExpression expr]
 :  ^(COL_RANGE (startExpr = col_ref[$plan])? DOUBLE_PERIOD (endExpr = col_ref[$plan])? )
    {
        SourceLocation loc = new SourceLocation( (LatinParserNode)$col_range.start );
 ...
 : ^( BIN_EXPR cond[$plan] e1 = expr[$plan] e2 = expr[$plan] )
   {
       ...
       $expr.setLocation( new SourceLocation( (LatinParserNode)$bin_expr.start ) );
   }
;
...
@init {
 ...
 : ^( CASE_EXPR ( ( ^( CASE_EXPR_LHS lhs = expr[$plan] { lhsExprs.add($lhs.expr); } ) )
                  ( ^( CASE_EXPR_RHS rhs = expr[$plan] { rhsExprs.add($rhs.expr); } ) )+ )+ )
    {
        ...
        $expr.setLocation( new SourceLocation( (LatinParserNode)$case_expr.start ) );
    }
;
...
@init {
 ...
 : ^( CASE_COND ^( WHEN ( cond[$plan] { conds.add($cond.expr); } )+ )
                ^( THEN ( expr[$plan] { exprs.add($expr.expr); } )+ ) )
    {
        ...
        $expr = currBinCondExpr;
        $expr.setLocation( new SourceLocation( (LatinParserNode)$case_cond.start ) );
    }
;
...
@init {
 ...
 :  ^( LIMIT rel ( INTEGER
   {
       $alias = builder.buildLimitOp( new SourceLocation( (LatinParserNode)$LIMIT ),
...
@init {
 ...
 : ^( SAMPLE rel ( DOUBLENUMBER
   {
       $alias = builder.buildSampleOp( new SourceLocation( (LatinParserNode)$SAMPLE ), $statement::alias,
           $statement::inputAlias, Double.valueOf( $DOUBLENUMBER.text ),
           new SourceLocation( (LatinParserNode)$DOUBLENUMBER ) );
   }
 | expr[exprPlan]
   {
       $alias = builder.buildSampleOp( new SourceLocation( (LatinParserNode)$SAMPLE ),
...
@after {
 ...
 {
	SourceLocation loc = new SourceLocation( (LatinParserNode) $rank_clause.start );
...
@init {
 ...
 : STAR {
		LogicalExpressionPlan plan = new LogicalExpressionPlan();
		builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$STAR ), plan, $GScope::currentOp, $statement::inputIndex, null, -1 );
		$plans.add( plan );
   }
...
@init {
 ...
 : ^( ORDER rel order_by_clause func_clause[FunctionType.COMPARISONFUNC]? )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$ORDER );
...
distinct_clause returns[String alias]
 : ^( DISTINCT rel partition_clause? )
   {
       $alias = builder.buildDistinctOp( new SourceLocation( (LatinParserNode)$DISTINCT ), $statement::alias,
...
cross_clause returns[String alias]
 : ^( CROSS rel_list partition_clause? )
   {
       $alias = builder.buildCrossOp( new SourceLocation( (LatinParserNode)$CROSS ), $statement::alias,
...
@after {
 ...
 : ^( JOIN join_sub_clause join_type? partition_clause? )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$JOIN );
...
join_type returns[JOINTYPE type]
 : QUOTEDSTRING
   {
       $type = builder.parseJoinType( $QUOTEDSTRING.text, new SourceLocation( (LatinParserNode)$QUOTEDSTRING ) );
   }
;
...
@init {
 ...
 : ^( UNION ( ONSCHEMA { onSchema = true; } )? rel_list )
   {
      $alias = builder.buildUnionOp( new SourceLocation( (LatinParserNode)$UNION ), $statement::alias,
...
@init {
 ...
 : ^( FOREACH rel foreach_plan )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$FOREACH );
...
@init {
 ...
 : ^( NESTED_PROJ
   ...
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$cr0.start );
...
@init {
 ...
 : ^( FILTER nested_op_input cond[plan] )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$FILTER );
...
@init {
 ...
 : ^( DISTINCT nested_op_input )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$DISTINCT );
...
@init {
 ...
 : ^( LIMIT nested_op_input ( INTEGER
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$LIMIT );
...
@init {
 ...
 : ^( CROSS nested_op_input_list )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$CROSS );
...
@init {
 ...
 : ^( GENERATE ( flatten_generated_item
   ...
   {
       builder.buildGenerateOp( new SourceLocation( (LatinParserNode)$GENERATE ),
...
@init {
 ...
 : col_ref[plan]
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$col_ref.start );
...
@init {
    ...
    SourceLocation loc = new SourceLocation( (LatinParserNode)$stream_clause.start );
}
                 ...
                 | IDENTIFIER
                   {
                       ...
                       if( cmd == null ) {
                           ...
                           throw new ParserValidationException( input,
                               new SourceLocation( (LatinParserNode)$IDENTIFIER ), msg );
                       }
                   }
                 ) as_clause? )
   {
...
@init {
    ...
    SourceLocation loc = new SourceLocation( (LatinParserNode)$mr_clause.start );
}
...
split_clause
 : ^( SPLIT
      rel
      {
          SourceLocation loc = new SourceLocation( (LatinParserNode)$SPLIT );
...
@init {
 ...
 : ^( SPLIT_BRANCH alias cond[splitPlan] )
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$alias.start );
...
alias_col_ref[LogicalExpressionPlan plan] returns[LogicalExpression expr]
 : GROUP
   {
       $expr = builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$GROUP ), $plan, $GScope::currentOp,
           $statement::inputIndex, $GROUP.text, 0 );
   }
 | CUBE
   {
       $expr = builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$CUBE ), $plan, $GScope::currentOp,
           $statement::inputIndex, $CUBE.text, 0 );
   }
 | IDENTIFIER
   {
       SourceLocation loc = new SourceLocation( (LatinParserNode)$IDENTIFIER );
 ...
 : DOLLARVAR
   {
       ...
       $expr = builder.buildProjectExpr( new SourceLocation( (LatinParserNode)$DOLLARVAR ), $plan, $GScope::currentOp,
 ...
 : literal
   {
       ...
       $expr.setLocation( new SourceLocation( (LatinParserNode)$const_expr.start ) );
   }
;
